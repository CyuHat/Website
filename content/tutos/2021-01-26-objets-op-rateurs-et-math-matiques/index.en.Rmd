---
title: "Objets, opérateurs et mathématiques"
author: Vestin Cyuzuzo Hategekimana
date: '2021-01-26'
slug: objets-op-rateurs-et-math-matiques
categories:
  - R
tags:
  - r
...
On utilise ça dans les scripts pour expliquer les lignes de codes, mais pas vraiment dans des lignes de commande dans la console. Vous pouvez aussi créer des mots ou chaines de caractères en les inscrivant entre guillemets: 
...
Naturellement, R est un semi-langage de programmation, il peut donc gérer des variables. Pour stocker une valeur dans une variable, il suffit juste d’indiquer le nom de la variable ainsi que la valeur que l'on souhaite lui attribuée et de mettre entre les deux (=), (<-) ou (->):
...
On peut également faire des calcules avec les variables ou tout autre sorte de manipulation :
...
Le prochain type d'objet abordé est le vecteur. Le vecteur est une série de valeur contenu dans le même objet. Pour le créer il faut utiliser la fonction d concaténation abrégée c() en séparant chaque valeur par une virgule dans R:
...
Ainsi, nous avons créé des vecteurs numériques (v1) et à chaine de caractères (v2 et v3). En effet, l'objet devient automatiquement une chaîne de caractères si au moins l'un de ses éléments est un caractère. Pour tester cela, nous pouvons utiliser la fonction class():
...
Ici, "numeric" signifie que c'est un objet numérique et "character" une chaîne de caractère...(Et là vous dites: "merci Einstein!", même s'il n'était pas traducteur). Vous pouvez testez cette fonction sur les autres objets créés (a,b,c et d).
...
Mais ça ne fonctionne pas lorsqu'il n'y a pas de diviseurs communs:
...
Pour gérer les objets vous pouvez utiliser ces 2 fonctions importantes: ls() et remove() ou rm() en abrégé. ls(), affiche tous les objets créés (laisser tel quel). remove() ou rm() supprime les objets sélectionnés. Exemple (Attention avant de lancer l’exemple assurer vous d’avoir enregistré tous les objets importants, puisque la dernière fonction les supprime tous):
...
Remarquez que nous pouvons le faire dans les deux sens (ascendant/descendant). Pour réaliser des séries plus personnalisées vous pouvez utiliser la fonction seq() en indiquant dans un premier temps le nombre de départ, puis le nombre d’arrivé, finalement indiquez la taille du saut après avoir écrit "by=":
...
**Sélection**
Il est possible de manipuler qu'une partie des vecteurs:
...
Gardez en mémoire le fait que cela sera très utile pour la manipulation de matrices et de structures de données, mais qu'il existe un packages qui gère mieux ce genre de situation (dyplr de tidyverse)
...
Suivant le code inscrit, R nous renvoi soit TRUE (Vrai) soit FALSE (Faux). En guise d'explication simple, il nous indique si l'expression ou la condition que nous lui avons proposé est vraie ou fausse. Dans notre exemple 2 n'est pas plus grand que 3 (donc faux) et 1 est bien inférieur à 100 (donc vrai). Cela marche aussi pour les vecteurs:
...
*Remarque: Les signes "|" (ou par élément) et "||" (ou simple) n'aparraissent pas dans le tableau à cause de leur particularité, mais ce sont bient ces signes.*
...
Les booléens sont un type d'objet qui prennent que deux types de valeurs vrai "TRUE" ou faux "FALSE". Et le tout s'écrit en majuscule:
...
Les deux valeurs peuvent être considéré comme des chiffres, là où TRUE vaut 1 et FALSE vaut 0.
...
Les booléens à eux seuls ne servent pas à grand chose, mais utilisés dans des conditions logiques (pas traîté dans ce tutoriel) ou dans des sélections, ils sont d'une grande efficacité. Voilà un exemple qui est loin de démontrer leur efficacité, mais simplement pour comprendre comment ils fonctionnent dans la sélection:
...
Ici nous présentons deux éléments utiles pour la sélection/modification. Premièrement nous présentons la fonction _%in%_ qui permet de contrôler si un élément est présent dans un ensemble d'éléments présentés. Il permet de demandé "Est-ce que tel élément est égal à l'un des éléments de la liste suivante". Voici une démonstration:
...
Deuxièmement, les vecteurs de booléens peuvent servir à modifier d'autres vecteurs et particulièrement grâce à la fonction _ifelse()_ qui permet de modifier un vecteur différemment en fonction de si un élément logique est respecte. Le premier paramètre de la fonction c'est l'opération logique, le deuxième, c'est le résultat à retourné si c'est vrai et le troisième c'est le résultat à retourné si c'est faux.
...
Dans cette partie, nous allons parler des listes, qui ne sont pas la même choses que les vecteurs. Même si elles sont similaires dans le fait qu'elles peuvent contenir une série d'élément dans un ordre précis, elles diffèrent sur ces points:

- Une liste peut contenir n'importe quelle types d'objets (chaîne de caractères, nombre, booléen, vecteurs, data frame, etc.) et peut en avoir différents types sans que ça ne pose un problème.
...
- Les éléments d'une liste ne sont pas directement manipulables, ni directement accessible contrairement aux vecteurs. Il faut les extraire d'une manière spéciale.
...
Si nous nommons les éléments d'une liste, il est possible de les récupérer avec leur noms:
...
Finalement, il est possible de récupérer des éléments imbriqué d'une liste dans une liste:
