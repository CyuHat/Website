---
title: "Statistique descriptive (1)"
...
Ces fonctions permettent de traiter les objets considérés (voire les transformer) en un type particulier de données. Lorsque nous voulons transformer nos variables en facteur, nous les transformons en variables catégorielle et c'est comme ça que R les traitera à l'avenir. Pour cela, nous allons stocker la transformation à la place des anciennes valeurs dans le tableau de données avec as.factor():
...
*Note: je ne recommande pas cette techniques, surtout lorsque vous avez plus d'objets à gérer.*

Pour générer des tableaux, il y a 2 types de fonction: table() et ftable(). Les deux donnent des résultats similaires, mais ftable est plus compact pour les tableaux avec beaucoup variables. Mais table() permet de diviser les tableaux. Au final chacun choisi selon ses goûts et selon la situation. Les tableaux sont utiles pour les variables catégorielles:
...
Mais conserver un objet contenant la table est bien plus pratique, puisque nous pouvons l'appeler en permanence pour d'autre fonction plus rapidement. Nous allons conserver "t1" pour l'instant.
...
Parfois nous souhaitons un tableau lisible. Nous souhaitons donc utiliser les pourcentages, il nous suffit juste de multiplier notre expression par 100:
...
Nous pouvons faire les tests statistiques pour les éléments partielles des variables. Par exemple quelle est la moyenne d'art des 10 premières personnes ou des individus 9, 52, 78, 104, 165, 222, 356 et 420: 
...
Remarque: Tout ce qui est valable pour l'une des fonctions statistiques présentée 
plus tôt, l'est aussi pour le reste des fonctions statistiques.

Nous pouvons aussi voire la moyenne des individus en info lorsque leur note dépasse 5:
...
Nous pouvons voir leur moyenne en anglais, lorsque les individu sont à la fois dans la classe C et ont 17 ans:

```{r}
mean(anglais[classe=="C" & age==17])
```

Lorsqu'on commence à utiliser des restrictions aussi précise, il est important de contrôler si la population dont on calcule le paramètre statistique est suffisamment grande avec la fonction length:
...
Nous allons maintenant voir comment gérer les valeurs manquantes. Nous allons générer une nouvelle variable complétement aléatoire concernant le nombre de cahier que les individus possèdent (cette information est complétement inutile au cas où vous ne l'auriez pas compris). Nous utiliserons cette variable pour apprendre à gérer les variables manquantes. Dans notre scénario, nous posons la question à nos répondant. Une partie d'entre eux hésite sur le nombre de cahier en leur possession, une autre le sait parfaitement et une autre pense "Franchement, je me demande à quoi ça sert ce cirque" et c'est justement cette partie qui ne sait pas le nombre de cahier en leur possession. En générale, lorsque l'information est manquante, la valeur de la variable devient NA. Nous allons générer cette variable et l'ajouter à notre base de données:
...
Remarque: Vous commencez gentiment à comprendre comment fonctionne la structure de R. C'est pourquoi, dans la suite, nous ne décortiquerons pas les fonctions et vous laissons l'exercice de les explorer en vous aidant de de la fonction help(), de votre bon sens et bien sûre d'internet.

Nous décidons de partir du principe que toutes les personnes qui n'ont pas pu nous donner un nombre de cahier n'en n'ont pas, donc zéro (C'est une décision très risquée en réalité, mais nous faisons ça en tant qu'exemple pour introduire le changement de valeur dans les variables). Pour faire cela nous utilise le code suivant: 
...
*Remarque: Ici nous n'avons pas utiliser la variable cahier du tableau de données ecole pour éviter d'avoir trop de changement.*
...
*Remarque: Puisque nous avons des valeurs manquantes dans nos données, nous devons les exclures de notre sélection pour que le code fonctionne, sinon vous aurez une erreure avec la fonction !is.na().*

Nous pouvons tout aussi bien exclure les valeurs manquantes de la variable ou de tous les tableau de données:
...
Bon, au fond cette variable ne nous sert à rien en dehors des exemples données, nous allons donc la supprimée:
...
Nous avons donc réussi à créer notre variable ordinale. L'avantage avec cette "class" en "numeric", c'est que R reconnait désormais qu'il existe une hiérarchie fix entre les différentes valeurs que peut prendre la variable.

Notons que pour tout ce que nous avons montré ici, il existe des bibliothèques de codes (library) téléchargeable qui permettent de simplifier les codes et de faire bien plus. Mais ce sera le sujet d'un futur tuto.
...
De manière générale, nous vous recommandons cette chaîne YouTube pour quelques fonctions particulière que nous n'aborderons pas dans nos tutos.
